...
class MemphisDB extends LevelDB{
    public function putArray($key, array $values, array $write_options = []){
        parent::put($key, json_encode($values), $write_options);
    }

    public function readArray($key, array $read_options = []){
        return json_decode(parent::get($key, $read_options));
    }
}
